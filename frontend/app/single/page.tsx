...
export default function Home() {
  ...
  return (
    <main className="app-surface h-dvh overflow-hidden px-2 py-2 sm:px-3 lg:px-6 lg:py-4">
      <div className="mx-auto grid h-full w-full max-w-6xl grid-rows-[auto_minmax(0,1fr)] gap-2 lg:gap-4">
        <AppHeader locale={locale} currentPage="single" onToggleLocale={() => setLocale(locale === "ja" ? "en" : "ja")} />

        <div className="grid min-h-0 grid-rows-[minmax(0,0.86fr)_minmax(0,1.14fr)] gap-2 md:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)] md:grid-rows-none lg:gap-4">
          <section className="app-panel grid min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-1.5 overflow-hidden p-2 sm:gap-2 sm:p-3 lg:gap-3 lg:p-4" aria-label={t.photoAria}>
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-blue-50 text-blue-600 sm:h-10 sm:w-10 sm:rounded-2xl">
                  <ScanText size={20} aria-hidden="true" />
                </span>
                <div>
                <p className="app-eyebrow hidden sm:inline-flex">{t.capturePanel}</p>
                <h1 className="app-heading sm:mt-2">{t.capture}</h1>
                <p className="sr-only">{t.captureHelp}</p>
                </div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <span className="grid h-9 w-9 place-items-center rounded-xl border border-blue-100 bg-blue-50 text-blue-600 sm:h-10 sm:w-10 sm:rounded-2xl" title={imageFile ? t.readyPhoto : t.noPhoto} aria-label={imageFile ? t.readyPhoto : t.noPhoto}>
            ...
            {previewUrl ? <p className="sr-only">{t.resultTools}</p> : null}
          </section>

          <section className="app-panel grid min-h-0 grid-rows-[auto_minmax(0,1fr)_auto_auto] gap-1.5 overflow-hidden p-2 sm:gap-2 sm:p-3 md:sticky md:top-2 lg:top-4 lg:gap-3 lg:p-4" aria-label={t.destination}>
            <div className="flex items-center gap-3">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-blue-50 text-blue-600 sm:h-10 sm:w-10 sm:rounded-2xl">
                <Navigation size={20} aria-hidden="true" />
              </span>
              <div>
              <p className="app-eyebrow hidden sm:inline-flex">{t.confirmPanel}</p>
              <h2 className="app-heading sm:mt-2">{t.destination}</h2>
            ...
            <button className="primary-action min-h-11 px-4 sm:min-h-12 lg:min-h-14" type="button" onClick={openMaps} disabled={!activeAddress} aria-label={t.mapsAria} title={t.mapsAria}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div
                className={[
                  "metric-card grid min-h-10 place-items-center gap-1 px-2 text-xs font-bold sm:min-h-12 lg:min-h-16",
